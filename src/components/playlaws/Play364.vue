					...
					<ul>
						<template v-for="item in pageData">
							<li :playPlId="item.playPlId">
								<!-- <strong :class="{'length':item.name.length>=3}">{{ item.name}}</strong><p>{{item.playPl}}</p> -->
								<strong :class="{'length':item.name.length>=3}">{{ hello(item.name)}}</strong><p>{{item.playPl}}</p>
							</li>
						</template>
					</ul>
				</div>
	...
	export default{
		...
		computed:{
			Getplay(){
				...
				if(filter)
					return filter[0].sscPlayPlList[3]
			},
			pageData(){
				const { two_title } = { ...this.$store.state.BuyNsidePage}
				...
				function count(obj){
					...
					$($oLi).find('li').each(function(index){
						if($(this).hasClass('color')){
							...
							if($(this).find('.length').length>0){
							zhushu.content='尾数-'+$(this).find('strong').text() // 号码
							}else{
