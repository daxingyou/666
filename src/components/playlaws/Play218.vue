		...
		<div class="machine-select">
			<h3>
				...
				<p>
					...
					<span class="five">机选一注</span>
				</p>
			</h3>
		</div>
		<div class="hcl-container">
			<div class="tips">第一位</div>
			<div class="number">
...
<script>
	export default{
		...
		mounted(){
			...
			$('.machine-select h3 p span.one').on('click',function() {
				var total = $('.hcl-container .number').eq(0).children('.item').length;
				var resuit = rand_one_zhu(total,1);
			...
			$('.machine-select h3 p span.five').on('click',function() {
				var total = $('.hcl-container .number').eq(0).children('.item').length;
            ...
            function check_total() {
                return $('.hcl-container .number .item.active').length;
            }
			//默认点击事件
