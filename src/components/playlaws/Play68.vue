	...
	mounted(){
		...
		_this.$nextTick(()=>{
			...
			function count(obj){
				...
				// 注数
				_this.setValue.notes=num*(num-1)*(num-2)*(num-3)/24
			...
			$(".machine-select h3 p .one").click(function(){
				...
				$(".machine-select ul li .number span").eq(arry[2]-1).addClass('color');
				$(".machine-select ul li .number span").eq(arry[3]-1).addClass('color');
