...
<script>
	export default {
		...
		methods:{
			...
			// 签到
			qiandao(){
				this.alert({result:1},this,'活动暂未开放！');
			},
			// 获取等级
			showUserPointLevel(){
				var _this=this;
				_this.ajax('showUserPointLevel',{
					uid:_this.uid,
					token:_this.token
				},
				data=>{
					console.log(data)
					var layerName=_this.$store.state.PersonalCenter.layerName

					if(data.level){
						_this.level=data.level
					}else{
						_this.level=data.description
					}
			...
			getHongbaoInfo(){
				...
				data=>{
					_this.alert(data,_this)
					if(data.hongbao){
						_this.envelopes=data.hongbao['hongbaoMoney'+(data.actTimes-1)]
					}
				})
			}
