		...
		<div class="wrap">
			...
			<ul>
				<li v-for="item in czList">
					<a @click="changeLink(item.payType,item.type,item.openType,item.url,item.payOnlineId)">
						<div class="pict">
...
<script>
	...
	export default{
		...
		methods:{
			changeLink(payType,type,openType,url,payOnlineId){
				...
				// 在线银行
				if(payType==4 && type==2 && url.length>15)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money
				...
				// QQ扫码
				if(payType==3 && type==2 && openType==0)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money

				// test_3
				if(payType==1 && type==2 && openType==1)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money

				// test
				if(payType==1 && type==2 && openType==0 && url.length>15)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money
				...
				// 支付宝wap-充值更快捷！
				if(payType==2 && type==2 && openType==0 && url.length>15)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money
				
				// 京东扫码
				if(payType==5 && type==2 && openType==0 && url.length>15)
					str=url+"?uid=" + this.uid + "&payOnlineId=" + payOnlineId + "&money=" + this.money
