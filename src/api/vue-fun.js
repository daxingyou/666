...
export function vuefun(Vue,options){
	...
	Vue.prototype.getDate=(val)=>{
		...
		// 今天
		if(val==1){
			obj.startTime=year+'-'+m+'-'+formatTwoNumber(d)+" 00:00:00"
			obj.endTime=year+'-'+m+'-'+formatTwoNumber(d+1)+" 00:00:00"
		}

		// 昨天	
		if(val==2){
			if(d-1==0){
				obj.startTime=year+'-'+formatTwoNumber(m-1)+'-'+"31 00:00:00"
				obj.endTime=year+'-'+m+'-'+formatTwoNumber(d)+" 00:00:00"
			}else{
				obj.startTime=year+'-'+m+'-'+formatTwoNumber(d-1)+" 00:00:00"
				obj.endTime=year+'-'+m+'-'+formatTwoNumber(d)+" 00:00:00"
			}
		}
		...
		if(val==3){
			var a=d+1-d;
			obj.startTime=year+'-'+ m +'-'+formatTwoNumber(a)+" 00:00:00"
			obj.endTime=year+'-'+ m +'-'+formatTwoNumber(d+1)+" 00:00:00"
		}
		return obj
	...
	Vue.prototype.subStr=function(str,num,offcet){
		...
		if(str){
			var new_str=str.substring(str.length-num)*1+offcet
	...
	Vue.prototype.downTime=function(time){
		if(time){
			...
			// return formatTwoNumber(day1)+':'+formatTwoNumber(hour)+':'+formatTwoNumber(minute)+':'+formatTwoNumber(second)
			return formatTwoNumber(hour)+':'+formatTwoNumber(minute)+':'+formatTwoNumber(second)
			// 时分秒
			// console.log(day1)
			// console.log(hour)
			// console.log(minute)
